...
import { GoodIcon, GameIcon } from './game-icons';
...
export function Orders({
  ...
  const orders = s.commerce.orders
    .filter((o) =>
      mode === '可接订单'
        ? o.status === 'offered' && o.postedDay === s.day
        : mode === '进行中'
          ? o.status === 'accepted'
          : ['delivered', 'failed', 'declined'].includes(o.status),
    )
    .toReversed();
...
export function Growth({ s, onBack }: { s: GameState; onBack: () => void }) {
  const [stored, setPage] = useWorkspaceValue<number>('growth.page', 0);
  const page = Math.min(Math.floor(stored), MILESTONES.length - 1);
  const milestone = MILESTONES[page];
  return (
    <section className="panel">
      <div className="panel-head">
        <h1>经营成长</h1>
      ...
      <div className="split">
        <div className="item-list">
          {MILESTONES.map((m, i) => (
            <button
              className={`list-item ${page === i ? 'selected' : ''}`}
              key={m.id}
              onClick={() => setPage(i)}
            >
              <strong>{m.name}</strong>
              <small>{s.commerce.milestones[m.id] ? '已达成' : '进行中'}</small>
            </button>
          ))}
        </div>
        <aside className="detail">
          ...
          <p>{milestoneProgress(s, milestone.id)}</p>
          <p>奖励：{milestone.reward}</p>
          {s.commerce.milestones[milestone.id] && (
            <div className="milestone-seal" aria-label="经营成就纪念">
              <GameIcon
                kind={
                  milestone.id === 'home'
                    ? 'mansion'
              ...
              <small>第{s.commerce.milestones[milestone.id]}日记</small>
            </div>
          )}
        </aside>
      </div>
