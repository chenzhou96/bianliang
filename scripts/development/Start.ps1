...
if (-not $ready) {
  if (-not (Test-Path -LiteralPath (Join-Path $projectRoot 'dist\client\index.html'))) { throw '缺少构建结果，请先按 README 构建项目。' }
  ...
  $process=Start-Process -FilePath $node -ArgumentList ('"'+$serverScript+'"') -WorkingDirectory $projectRoot -WindowStyle Hidden -PassThru -RedirectStandardOutput (Join-Path $runtime 'server.log') -RedirectStandardError (Join-Path $runtime 'server-error.log')
